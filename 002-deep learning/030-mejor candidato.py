...
def visionArtificial(rutaimagen):
    ...
    archivo.write(rutaimagen+"|"+str(diccionarioporcentajes)+"\n")
    archivo.close()
    #imagendestino.show()

def visionArtificialLee(rutaimagen):
    imagen = Image.open(rutaimagen)
    imagengris = imagen.convert("L")
    bordes = imagengris.filter(ImageFilter.FIND_EDGES)
    enhancer = ImageEnhance.Contrast(bordes)
...
##for file in files:
##    visionArtificial(file)

print(visionArtificialLee("muestraprueba2.jpg"))
muestra = visionArtificialLee("muestraprueba2.jpg")
memoria= "memoria.txt"
...
with open(memoria, "r") as file:
    ...
    for line in file:
        suma = 0;
        lineadiccionario = ast.literal_eval(line.split("|")[1])
        angulos = [-180,-135,-90,-45,0,45,90,135,180]
        for angulo in angulos:
            suma += abs(muestra[angulo]-lineadiccionario[angulo])
        print("Para la imagen "+line.split("|")[0]+" la suma es de "+str(suma))
        if suma < mejorsuma:
            print("es ok")
            mejorsuma = suma
            mejorcandidato = line.split("|")[0]

print("El mejor candidato es: "+mejorcandidato)

imagenorigen = "muestraprueba2.jpg"
imagepilo = Image.open(imagenorigen)
imagepilo.show()
               
imagepil = Image.open(mejorcandidato)
imagepil.show()
